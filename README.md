...
- 단순 IN/OUT 카운트 차이 방식은 재입차 케이스에서 오차가 생겨 NOT EXISTS 서브쿼리 기반으로 현재 주차 대수를 정확하게 계산
- 회원가입/차량 등록 신청 시 관리자 알림을 DB에 저장하고 프론트에서 폴링으로 조회 — SSE/WebSocket 대비 구현 복잡도를 고려해 폴링 방식 선택
- JWT를 HttpOnly 쿠키로 관리해 XSS 공격 방어 및 만료 시 자동 재발급 인터셉터 적용
- GX 예약은 PENDING 상태로 저장 후 관리자 승인 시 7명 이상이면 전원 CONFIRMED, 미만이면 전원 CANCELLED 일괄 처리
- 좌석 있는 시설은 DB UNIQUE 제약으로, 좌석 없는 시설(헬스장/GX)은 트랜잭션으로 동시성 제어
